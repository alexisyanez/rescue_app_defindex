...
export async function totalUnderlyingInCometPool(): Promise<bigint> {
  console.log("Fetching LP Backstop Balance...");
...
export async function getBackstopBalance(): Promise<bigint> {
  console.log("Evaluating vault risk...");
  const totalUnderlying = await totalUnderlyingInCometPool();
  console.log("Total Underlying in Comet Pool:", totalUnderlying.toString());
  const lpBackstopBalance = await LPBackstopBalance();
  console.log("LP Backstop Balance:", lpBackstopBalance.toString());
  const totalSupply = await cometPoolTotalSupply();
  console.log("Comet Pool Total Supply:", totalSupply.toString());
...
export async function evaluateVaultRisk(): Promise<boolean | undefined> {
  try {
    ...
    console.log("Backstop Balance:", backstopBalance.toString());
    const vaultBalance = await vaultBalanceInStrategy();
    console.log("Vault Balance in Strategy:", vaultBalance.toString());
    ...
    console.error("Error evaluating vault risk:", JSON.stringify(error));
  }
}
